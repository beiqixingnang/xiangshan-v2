...
IEEE field decode, integer-to-float conversion (RNE/RTZ/RDN/RUP/RMM control),
and deterministic add/multiply operation shells used by the XiangShan FPU.
"""
...
class FudianFpu(Elaboratable):
    """Combinational decode and integer-to-float conversion boundary."""
...
def build_verilog(configuration: FpuConfig | Mapping[str, Any] | None, injected_dependencies: Mapping[str, Any]) -> str:
    ...
    top = FudianFpu(cfg); return verilog.convert(top, name=name, ports=[top.clock, top.reset, top.input, top.rounding, top.output, top.inexact, top.is_nan, top.is_inf, top.is_zero], emit_src=False)
