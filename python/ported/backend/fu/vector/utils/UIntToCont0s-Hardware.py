"""UInt to a low-contiguous-zero mask.

Amaranth port of ``UIntToContLow0s`` from the pinned XiangShan snapshot.
"""
...
# Module Contract
# =============================================================================
# ``UIntToContLow0s`` returns (2**uintWidth - 1) bits. For input ``n`` the low
# ``n`` bits are zero and all higher bits are one; zero yields all ones and
# the maximum input yields all zeros. ``UIntToContHigh0s`` reverses the bits.
__all__ = [
    "UIntToContConfig",
    "UIntToContLow0s",
    "UIntToContHigh0s",
...
def contiguous_low0s(data: Any) -> Any:
    """Recursively build the low-contiguous-zero mask."""

    width = data.shape().width
    if width < 1:
        raise ValueError("data width must be positive")
    if width == 1:
        return cast(Any, Mux(bit_view(data, 0), zeros_constant(1),
                             ones_constant(1)))

    half = 1 << (width - 1)
    low = contiguous_low0s(data.bit_select(0, width - 1))
    ...
    return cast(Any, Mux(
        ...
        Cat(zeros_constant(half), low),
        Cat(low, ones_constant(half)),
    ))


class UIntToContLow0s(Elaboratable):
    """Generate ``2**uintWidth - 1`` low-contiguous-zero bits."""
    ...
    def elaborate(self, platform):
        module = Module()
        module.d.comb += self.dataOut.eq(contiguous_low0s(self.dataIn))
        return module


def UIntToContHigh0s(uint: Any) -> Any:
    """Return the bit-reversed low-zero mask for an existing value."""

    width = uint.shape().width
    return reverse_value(contiguous_low0s(uint), (1 << width) - 1)
...
def build_verilog(
    ...
    name: str = "UIntToContLow0s",
) -> str:
    """Emit deterministic Verilog for the low-zero implementation."""
