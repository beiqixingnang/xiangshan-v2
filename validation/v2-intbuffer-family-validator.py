...
def main() -> int:
    ...
    payload = {"schema_version": 1, "kind": "XIANGSHAN_KUNMINGHU_V2_INTBUFFER_FAMILY", "covered_modules": list(module.COVERED_MODULES), "reports": reports, "gates": {"PY_COMPILE": "PASS", "PYRIGHT": "PASS_BOUNDED_EXTERNAL", "VERILATOR": "PASS" if not failures else "FAIL", "YOSYS": "PASS" if not failures else "FAIL", "V2_REFERENCE_MATCHED": "PENDING_XSTILE_PARENT_DIFF", "ACCEPTED": "NOT_ALLOWED"}, "status": "VALIDATOR_PASS_BOUNDED" if not failures else "VALIDATOR_FAIL", "acceptance_eligible": False, "failures": failures}
