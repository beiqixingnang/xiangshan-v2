...
def main() -> int:
    ...
    # Keep the standard batch evidence split so the coordinator can audit the
    # family without interpreting one monolithic result file. / 保持标准批次证据拆分，便于协调器独立审计 family。
    contract = {"schema_version": 1, "kind": "XIANGSHAN_KUNMINGHU_V2_UTILITY_CONTRACT_AUDIT", "batch_id": "V2-DEPENDENCY-UTILITY-001", "target": evidence["target"], "static": static, "gates": evidence["gates"], "acceptance_eligible": False}
    coverage = {"schema_version": 1, "kind": "XIANGSHAN_KUNMINGHU_V2_UTILITY_COVERAGE", "batch_id": "V2-DEPENDENCY-UTILITY-001", "source_file_count": 61, "target": evidence["target"], "observed_operations": ["PriorityMuxDefault", "CircularQueuePtr", "ECC parity", "CriticalErrorUtils", "PerfCounterUtils"], "direct_vectors": direct["vectors"], "differential_vectors": diff["vectors"], "gates": evidence["gates"], "acceptance_eligible": False}
    mapping = {"schema_version": 1, "kind": "XIANGSHAN_KUNMINGHU_V2_UTILITY_MAPPING", "batch_id": "V2-DEPENDENCY-UTILITY-001", "family_id": "dependency.utility", "source_root": SOURCE_ROOT.relative_to(ROOT).as_posix(), "target": evidence["target"], "disposition": "AGGREGATED_FAMILY_BOUNDARY", "status": "PASS_BOUNDED_FAMILY", "gates": evidence["gates"], "acceptance_eligible": False}
    (ROOT / "validation" / "v2-utility-family-contract-audit.json").write_text(json.dumps(contract, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "validation" / "v2-utility-family-coverage-manifest.json").write_text(json.dumps(coverage, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "validation" / "v2-utility-family-mapping-update.json").write_text(json.dumps(mapping, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
