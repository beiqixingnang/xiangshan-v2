"""Freeze explicit locked Debug-family ports."""
...
def width(token: str) -> int:
    """Parse Verilog range width."""

    m = re.search(r"\[\s*(\d+)\s*:\s*(\d+)\s*\]", token)
    return abs(int(m.group(1)) - int(m.group(2))) + 1 if m else 1


def main() -> None:
    """Replace the marked catalog block from locked hierarchy."""
